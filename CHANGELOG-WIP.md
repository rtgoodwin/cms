...
- Added `craft\queue\QueueWithReleaseInterface`. ([#16672](https://github.com/craftcms/cms/pull/16672))
- Added `craft\services\Search::indexElementIfQueued()`.
- Added `craft\services\Search::queueIndexElement()`.

### System
- `craft\queue\Queue::release()` and `releaseAll()` now call `release()` and `releaseAll()` on the proxied queue if it implements `craft\queue\QueueWithReleaseInterface`. ([#16672](https://github.com/craftcms/cms/pull/16672))
